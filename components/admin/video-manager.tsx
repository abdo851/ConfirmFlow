...
function VideoForm({ video, onCancel }: { video: VideoBlock | null; onCancel: () => void }) {
  ...
  const [placement, setPlacement] = useState<VideoBlock["placement"]>(video?.placement ?? "dashboard_top");
  ...
  return (
    ...
    >
      ...
      <label className="flex flex-col gap-1.5 text-sm font-medium">
        ...
        >
          {(["landing_hero", "landing_below_hero", "onboarding_top", "dashboard_top", "global"] as const).map(
            (value) => (
