    ...
    if (table === "meta_conversion_deliveries") {
      ...
      return {
        ...
        update: (values: Partial<DeliveryRow>) => {
          const updateFilters: Record<string, string | string[]> = {};
          const builder = {
            eq(field: string, value: string) {
            ...
            in(field: string, statuses: string[]) {
              updateFilters[field] = statuses;
              return {
                select: () => ({
                  maybeSingle: async () => {
                    const delivery = deliveries.get(updateFilters.id as string);
                    if (
                      !delivery ||
                      !Array.isArray(updateFilters.status) ||
                      !updateFilters.status.includes(delivery.status)
                    ) {
