...
if (isDirectRun && !process.env.GOOGLE_EVENT_V2_LOADER) {
  process.env.GOOGLE_EVENT_V2_LOADER = "1";
  ...
  const { data: orderRow, error: orderRowError } = await supabase
    .from("orders")
    .select("id, external_order_id, order_number, total_amount_minor, currency")
    .eq("id", order.id)
    .maybeSingle();

  if (orderRowError || !orderRow) {
    console.error("Unable to read the order used for the payload.");
    console.error(orderRowError?.message ?? "missing");
    process.exit(1);
  }
  ...
  const { error: resetError } = await supabase
    ...
    .update({ status: "failed", last_error: null, sent_at: null })
    .eq("order_id", order.id)
    .eq("event_type", "Purchase")
    .eq("status", "sent");

  if (resetError) {
    console.error("Unable to reopen the previous Google delivery.");
