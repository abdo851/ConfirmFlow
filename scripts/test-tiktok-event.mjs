...
if (isDirectRun && !process.env.TIKTOK_EVENT_LOADER) {
  process.env.TIKTOK_EVENT_LOADER = "1";
  ...
  const deliveryModule = await import(
    pathToFileURL(join(root, "lib/integrations/tiktok/delivery/deliver-purchase.ts")).href
  );
  const result = await deliveryModule.processTikTokPurchaseDelivery({ order_id: order.id });
