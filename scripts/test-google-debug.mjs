...
if (isDirectRun && !process.env.GOOGLE_DEBUG_LOADER) {
  process.env.GOOGLE_DEBUG_LOADER = "1";
  ...
  const { data: deliveries, error: deliveriesError } = await supabase
    ...
    .select("id, order_id, event_id, status, attempts, sent_at, last_error, response_body, created_at")
    .order("created_at", { ascending: false })
    .limit(3);

  if (deliveriesError) {
    console.error("Unable to read google_conversion_deliveries.");
    console.error(deliveriesError.message);
    process.exit(1);
  }

  console.log("DELIVERIES");
  console.log(redact(JSON.stringify(deliveries ?? [], null, 2)));

  const latest = deliveries?.[0];
  if (!latest) {
    console.log("No Google delivery row to debug.");
    process.exit(0);
  }
  ...
  if (orderError || !order) {
    console.error("Unable to read the delivered order.");
    console.error(orderError?.message ?? "missing");
    process.exit(1);
  }
  ...
  const strictResult = await clientModule.sendGA4Event({
    measurement_id: eligibility.connection.measurementId,
    api_secret: eligibility.connection.apiSecret,
    event: strictPayload,
    debug: true,
  ...
  console.log(`strict_debug_http_status: ${strictResult.status}`);
  console.log("STRICT_DEBUG_RESPONSE");
  console.log(redact(JSON.stringify(strictResult.body, null, 2)));
}
